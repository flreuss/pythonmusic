...
.. important:: 
   The parameters after ``type`` are keyword arguments (``kwargs``). This means that you need to pass the labels defined in the table
   below as keywords. For instance:

   .. code-block:: python
      
      note_on = MidiMessage(NOTE_ON, channel=0, note=C4, velocity=MF)
      note_off = MidiMessage(NOTE_OFF, channel=0, note=C4, velocity=MF)
      cc_change = MidiMessage(CONTROL_CHANGE, channel=0, control=MOD_WHEEL, value=10)

  .. automethod:: pythonmusic.io.MidiMessage
     :no-index:
