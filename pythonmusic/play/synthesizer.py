...
#
#
# class SynthEffect(ABC):
#     def note_on(self, key: int): ...
#     def note_off(self, key: int): ...
#     def apply(self, input: NDArray[np.float32], key: int) -> NDArray[np.float32]: ...
#
#
# class DecayEffect(SynthEffect):
#     def __init__(
#         self,
#         decay: int,
#         target: float = 0.0,
#     ):
#         self._decay: list[float] = [decay] * 128
#         self._target = target
#
#     @classmethod
#     def from_seconds(
...
#         sample_rate: int = AUDIO_STREAM_DEFAULT_SAMPLE_RATE,
#     ) -> Self:
#         return cls(seconds_to_samples(duration, sample_rate), target)
#
#     def apply(self, input: NDArray[np.float32], key: int) -> NDArray[np.float32]:
#         # if decaul already reached
